...
class MyHomeScreenWidgets {
  static Widget cardWithImage(BuildContext context, bool isLeft,
      String imageResorce, String text, String type) {
    return Container(
      margin: EdgeInsets.only(left: isLeft ? 5 : 0),
      child: Card(
        shape: RoundedRectangleBorder(
          side: const BorderSide(
              color: ColorResource.COLOR_APP_TEXT_FIELD_BORDER, width: 1),
          borderRadius: BorderRadius.circular(10),
        ),
        child: Container(
            padding: EdgeInsets.all(16),
            height: type == "TABLET" ? 240 : 160,
            width: type == "TABLET" ? 240 : 160,
            child: Column(
              children: [
                Image.asset(
                  imageResorce,
                  height: type == "TABLET" ? 170 : 90,
                  width: type == "TABLET" ? 170 : 90,
                ),
                if (type != "TABLET")
                  SizedBox(
                    height: 10,
                ...
                Expanded(
                  ...
                  style: type == "TABLET"
                      ? CustomStyle.size22w400CAPPTXTCOLOR()
                      : CustomStyle.size18w400CAPPTXTCOLOR(),
                ))
              ],
            )),
      ),
    ...
    return Row(
      ...
      children: [
        Text(
          ...
          style: CustomStyle.size14w400CAPPCOLOR(),
        ),
        const SizedBox(width: 10),
        Switch(
            value: bloc.languageValue ?? false,
            activeColor: ColorResource.COLOR_APP_TEXT_FIELD_BORDER,
            onChanged: (bool) => {
                  if (bool == true)
                    {
